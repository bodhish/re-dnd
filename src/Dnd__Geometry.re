...
let getViewport = () => {
  ...
  Dimensions.{
    width: element |> HtmlElement.clientWidth |> float_of_int,
    height: element |> HtmlElement.clientHeight |> float_of_int,
  };
};
...
let isAfore = (~subject: Rect.t, ~comparand: Rect.t, ~axis: Axis.t) =>
  ...
  | X =>
    ...
    let subjectCenter = subject.left +. subjectWidth /. 2.0;
    let comparandCenter = comparand.left +. comparandWidth /. 2.0;
  ...
  | Y =>
    ...
    let subjectCenter = subject.top +. subjectHeight /. 2.0;
    let comparandCenter = comparand.top +. comparandHeight /. 2.0;
